@extends('layouts.book') @section('main')
<link
            ...
            <div id="update-description-modal" aria-labelledby="modal-title" role="dialog" aria-modal="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full" >
                <div class="relative p-4 w-full max-w-md max-h-full">
                    ...
                    <div class="relative bg-white rounded-lg shadow bg-gray-700"  >
                        ...
                        <!-- Modal body -->
                        <div class="p-4 md:p-5">
                            <form action="{{ route('user.update.description') }}" method="POST">
                                @csrf
                                <textarea id="update-description-textarea" name="description" class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none" rows="4" placeholder="Enter new description...">{{ $user->description }}</textarea>
                    ...
                    <div class="w-full">
                        <dl class="text-gray-900 divide-y divide-gray-200 text-white divide-gray-700">
                            ...
                            <div class="flex flex-col py-3">
                                ...
                                <dd class="text-lg font-semibold">
                                    <form action="{{ route('user.update.gender') }}" method="POST">
                                        @csrf
                            ...
                            <div class="flex flex-col py-3">
                                ...
                                <dd class="text-lg font-semibold">
                                    <form action="{{ route('user.update.address') }}" method="POST">
                                        @csrf
                                        <input type="text" name="address" value="{{ $user->address }}" class="w-full py-2 px-4 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-500 dark:bg-gray-800 dark:text-white" onclick="document.getElementById('update-address-btn').style.display = 'block';" style="background-color: transparent;">
                                        <div class="mt-2">
                            ...
                            <div class="flex flex-col py-3">
                                ...
                                <dd class="text-lg font-semibold">
                                    <form action="{{ route('user.update.phone') }}" method="POST">
                                        @csrf
                                        <input type="text" name="phone" value="{{ $user->phone }}" class="w-full py-2 px-4 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-500 dark:bg-gray-800 dark:text-white" onclick="document.getElementById('update-phone-btn').style.display = 'block';" style="background-color: transparent;">
                                        <div class="mt-2">
